    ...
    void OnGUI()
    {
        ...
        switch (this.step)
        {
            case STEP.PLAY:
                ...
                float blast_time = GAME_OVER_TIME - this.step_timer;  // 제한 시간에 도달할 때까지 남은 시간을 표시.
                GUI.Label(new Rect(pos_x, pos_y + 64, 200, 20), blast_time.ToString("0.00"));
                break;
            case STEP.CLEAR:
                GUI.color = Color.black;
                GUI.Label(new Rect(pos_x, pos_y, 200, 20), "탈출" + this.clear_time.ToString("0.00"), guistyle);  // 클리어 메시지와 클리어 시간 표시.

                pos_y -= 32;
                int ct = (int)clear_time; // 클리어 시간(float)를 int로 변환.
                if (ct > 50)
                { // 50초〜제한시간내.
                    GUI.Label(new Rect(pos_x, pos_y, 200, 20), "아슬아슬탈출! 50초 이내를 목표로 하세요!");
                }
                else if (ct > 40)
                { // 40〜50초.
                    GUI.Label(new Rect(pos_x, pos_y, 200, 20), "멋져요! 40초 안을 목표로 하세요!");
                }
                else if (ct > 30)
                { // 30〜40초.
                    GUI.Label(new Rect(pos_x, pos_y, 200, 20), "대단해요！30초 이내를 목표로 하세요!");
                }
                else
                { // 30초이내！
                    GUI.Label(new Rect(pos_x, pos_y, 200, 20), "빨라요! 플라플라 마스터! ");
                }
                break;
            case STEP.GAMEOVER:
                GUI.color = Color.black;
                GUI.Label(new Rect(pos_x, pos_y, 200, 20), "게임 오버", guistyle);  // 게임 오버 메시지를 표시.
                break;
        }
    }
}
